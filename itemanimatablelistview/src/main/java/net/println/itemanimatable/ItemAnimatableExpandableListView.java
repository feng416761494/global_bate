...
//                        }
        ...
        getViewTreeObserver().addOnPreDrawListener(new OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                ...
                for (int i = 1; i <= childCount; i++) {
                    ...
                    int groupTop = groupView.getTop();
                    int childViewTop = childView.getTop();
                    int location = groupTop - childViewTop;
                    //2017/4/10  冯井起 更改lirbary源码
                    Animator animator = ObjectAnimator.ofFloat(childView, "translationY", groupView.getBottom() - childView.getTop(), 0);
    ...
    public void animateCollapse(final int group){
        ...
        for (int i = 1; i <= childCount; i++) {
            ...
            Animator animator = ObjectAnimator.ofFloat(childView, "translationY", 0, groupView.getBottom() - childView.getTop());
            if(shouldAddCallback){
                animator.addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
